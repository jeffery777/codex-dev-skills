...
For the current non-state-changing Desktop runtime wrapper V1 helpers and CLI usage examples, see [docs/desktop-runtime-wrapper-v1-plan.md](docs/desktop-runtime-wrapper-v1-plan.md). The capability discovery helper normalizes caller-supplied documented metadata only, and the planner can accept that normalized output as `capability_evidence` to produce dry-run, fallback, or stopped evidence. These helpers do not call Desktop thread tools, read Desktop private runtime state, or add a daemon, MCP server, app-server client, sidecar, background service, skill, catalog item, or installer entry.
