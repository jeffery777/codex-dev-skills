...
For the current non-state-changing Desktop runtime wrapper V1 helpers and CLI usage examples, see [docs/desktop-runtime-wrapper-v1-plan.md](docs/desktop-runtime-wrapper-v1-plan.md). The planner helper prepares dry-run or fallback evidence only, and the capability discovery helper normalizes caller-supplied documented metadata only. These helpers do not call Desktop thread tools, read Desktop private runtime state, or add a daemon, MCP server, app-server client, sidecar, background service, skill, catalog item, or installer entry.
