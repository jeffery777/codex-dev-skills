...
For the completed Desktop runtime wrapper V1 planner, capability metadata normalization, contract comparison, create-thread preflight, read-thread preflight, evidence pipeline, session compatibility status validation, first-use handshake, session-scoped compatibility cache, create-thread authorization/evidence gate, create-thread executor boundary proposal, create-thread executor shell, single documented create-thread callable executor, and create-thread callable wiring-boundary helpers, see [Desktop Runtime Wrapper V1 Feasibility And Implementation Plan](desktop-runtime-wrapper-v1-plan.md). The executor and wiring helpers are still non-live by default: CLI use falls back without an injected runner or caller-supplied descriptor, and `ready` means only that a caller-injected documented callable adapter contract completed with `desktop_runtime_call_performed: false` or that a documented descriptor was converted into callable wiring readiness.
