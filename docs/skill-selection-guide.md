...
| Shared orchestration selected a bounded task and the user wants one separate, resumed, or interactively forked Codex CLI session. | `cli-session-handoff` | Thin CLI control-plane adapter over stable non-interactive start/resume plus a manual exact-UUID interactive-fork handoff; the executor path validates the exact clean worktree and sandbox, while fork directory reuse remains explicit. |
