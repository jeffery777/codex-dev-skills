...
Before a future true documented `create_thread` callable is wired, use the V1 create-thread executor shell helper only as an implementation-surface check. It requires ready executor boundary proposal evidence, exact create-thread action/tool evidence, repo/remote/branch/expected-head target evidence, prompt evidence, an executor-shell-only human approval marker, blocked external-write and destructive-action boundaries, `runtime_call_performed: false`, no Desktop private runtime state read evidence, `surface_only: true`, `runtime_call_authorized: false`, a non-executed callable descriptor or injected-adapter placeholder, and call-site contract evidence for target identity, authorization intent, permission/auth failure classification, response shape, returned thread id, and returned status. `ready` means only that a human can consider separately approving one future documented `create_thread` wiring slice; it does not call or authorize `create_thread`, and prior proposal/gate/cache/preflight evidence cannot replace actual call-site target validation, permission handling, or response validation.

For the current V1 create-thread documented callable executor helper, use it only with ready executor shell evidence and an explicit caller-injected documented adapter. It rechecks target identity and authorization intent inside the helper, classifies auth/permission failures, validates response shape, returned thread id, and returned status, and requires the adapter to report `desktop_runtime_call_performed: false`, no private runtime state read, and no external write. CLI use without an injected runner returns `fallback`. `ready` is injected adapter contract completion only; true Desktop runtime `create_thread` callable injection or use still needs separate human approval and a runtime-provided documented callable. The next live wiring slice may connect at most one documented `create_thread` tool path.
