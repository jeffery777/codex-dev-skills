...
由同目錄 `reusable-workflow-contract.md` 的觸發表選讀適用章節；不是整份預載。
以下保留原完整規則。入口核心義務始終適用，觸發成立時在相依操作前讀取細則。
文件中的 `docs/` 路徑在 source/plugin 從套件根目錄解析；filesystem 安裝
從 `${CODEX_TEMPLATES_DIR:-$HOME/.codex/templates}` 解析。
...
驗收失敗或反覆返工時，按需讀取同目錄 `model-selection-policy.md` 的
「返工分類、升級與續行」：合理修正後仍失敗就重評方法與能力；返工門檻不
新增停止任務的上限。父代理保留路由、資格、整合及續行責任。
