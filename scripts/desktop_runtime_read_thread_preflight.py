#!/usr/bin/env python3
"""Read-thread runtime-call preflight helper.

This helper is intentionally non-state-changing. It checks whether caller-
supplied evidence is ready for a future, separately approved ``read_thread``
...
EXPECTED_CLASSIFICATION = "read-only"
...
def _fallback_prompt(request: dict[str, Any], reason: str) -> str:
    ...
    thread_id = _get(request, "target.thread_id") or "unknown thread id"
    summary = _get(request, "read_request.summary") or "No read summary provided."
    expected_fields = _get(request, "read_request.expected_fields") or []
    ...
    return "\n".join(
        [
            ...
            "Use the request below only where read_thread is intentionally available and separately approved, then return the read-only evidence here for integration review.",
            "",
            f"Fallback reason: {reason}",
            "Requested target action: read-thread",
            f"Repository: {repo}",
            f"Remote: {remote}",
            f"Branch: {branch}",
            ...
            "",
            "Expected fields:",
            ", ".join(expected_fields) if isinstance(expected_fields, list) else str(expected_fields),
        ]
    )
...
def _base_response(
    ...
    return {
        ...
        "readiness_meaning": (
            ...
            "read-only runtime call; it does not mean read_thread was called, "
            "a Desktop thread was read, or any external write is authorized."
        ),
        "target_evidence": {
            "repo": _get(request, "target.repo"),
            "remote": _get(request, "target.remote"),
            "branch": _get(request, "target.branch"),
            "thread_id": _get(request, "target.thread_id"),
        },
        "read_request_evidence": {
            "summary_present": not _is_missing(_get(request, "read_request.summary")),
            "summary": _get(request, "read_request.summary"),
            "expected_fields": _get(request, "read_request.expected_fields") or [],
        },
        "contract_evidence": {
...
def _forbidden_evidence_hits(request: dict[str, Any]) -> list[str]:
    ...
    read_text = []
    read_text.extend(_iter_strings(_get(request, "read_request.summary")))
    read_text.extend(_iter_strings(_get(request, "read_request.expected_fields")))
    hits.extend(_contains_any(_iter_strings(_get(request, "read_request.expected_fields")), PRIVATE_RUNTIME_HINTS))
    hits.extend(_contains_private_runtime_request(read_text))
    return hits
...
def _validate_capability_record(
    ...
    if capability.get("tool_or_api") != TOOL_OR_API:
        return _stopped(
            ...
            "read-thread capability must use read_thread.",
            capability,
        )
    if capability.get("classification") != EXPECTED_CLASSIFICATION:
        return _stopped(
            ...
            "read-thread capability classification must be read-only.",
            capability,
        )
    ...
    if capability.get("capability_source") not in CAPABILITY_SOURCES:
        return _stopped(
            ...
            "read-thread capability_source is not a recognized documented source.",
            capability,
        )
    if _is_missing(capability.get("contract_version")) or not _valid_iso_date(capability.get("last_verified")):
        return _stopped(
            request,
            "missing_contract_evidence",
            "read-thread capability requires contract_version and YYYY-MM-DD last_verified.",
            capability,
        )
    return None
    ...
    for record_name, record in (("old_contract", old_contract), ("new_capability", new_capability)):
        if record.get("action") != TARGET_ACTION:
            return _stopped(
                ...
                f"{record_name}.action must be read-thread.",
                capability,
            )
        if record.get("tool_or_api") != TOOL_OR_API:
            return _stopped(
                request,
                "tool_or_api_mismatch",
                f"{record_name}.tool_or_api must be read_thread.",
                capability,
            )
        if record.get("classification") != EXPECTED_CLASSIFICATION:
            return _stopped(
                request,
                "classification_mismatch",
                f"{record_name}.classification must be read-only.",
                capability,
            )
...
def preflight_read_thread(request: dict[str, Any]) -> dict[str, Any]:
    """Classify read_thread readiness evidence without calling the runtime."""
    ...
    if thread_action_authorized is not None and thread_action_authorized is not False:
        return _stopped(
            request,
            "runtime_call_authorization_out_of_scope",
            "read-thread preflight records readiness only; runtime call authorization must remain separate.",
        )
    ...
    expected_fields = _string_list(_get(request, "read_request.expected_fields"))
    if expected_fields is None:
        return _stopped(
            request,
            "validation_error",
            "read_request.expected_fields must be a non-empty string list.",
        )
    ...
    if request.get("target_action") != TARGET_ACTION:
        return _stopped(
            request,
            "validation_error",
            "Only read-thread preflight is supported by this helper.",
        )
    ...
    if selection_status == "fallback":
        return _fallback(
            request,
            "missing_capability",
            "Normalized read-thread capability evidence is unavailable.",
            None,
    ...
    if capability is None:
        return _fallback(request, "missing_capability", "read-thread capability is missing.")
    ...
    response["result"]["residual_risk"] = [
        ...
        "A future read-only runtime call still requires separate approval at the call site.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        response = preflight_read_thread(request)
